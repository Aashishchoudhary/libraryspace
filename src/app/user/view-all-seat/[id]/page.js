...
function View_all_seat({params :{id}}) {
  ...
  const fetchData = async () => {
    const response = await axios.get(`${url}/all-seat-reservation-view/${id}/`, {
      headers: {
  ...
  return (
    ...
    {filteredDataSource?.map(item => (
      <button key={item.seat_num}
        
        
        className={[styles.container ,item.seat_num%2 ? styles.evenStyle : styles.oddStyle]}>
        <div key={item.id}>
          <p className={styles.seatNumber}>{item.seat_num}</p>
          {item?.data?.map(x => (
            <div key={x.id}>
              <p className={styles.name}>{x.name.slice(0,1).toUpperCase()+x.name.slice(1)}</p>
