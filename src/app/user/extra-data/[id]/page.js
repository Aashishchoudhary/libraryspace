...
function ExtraStudent({ params: { id } }) {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${url}/extra-student-view/${id}/`, {
        headers: {
     ...
     console.log(res)
      ...
      res.data[0]["dob"]&&setDob(
         yyyymmdd(new Date(res.data[0]["dob"])) 
      );
      
      res.data[0]["end_date"]&&setEndDate(
           yyyymmdd(new Date(res.data[0]["end_date"]))
         
      );
      res.data[0]['start_date']&& setStartdate(yyyymmdd(new Date(res.data[0]['start_date'])))
